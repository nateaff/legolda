...
```{r other-evaluation, eval=TRUE, fig.height = 4, fig.width = 7}
```
...
devtools::load_all()
knitr::read_chunk(here::here("code", "coherence.R"))
```
...
```{r plot-external-scores, eval = TRUE, echo = FALSE, fig.height = 4,fig.width = 7}
```


```{r read-topic-distribution, eval = TRUE, echo = FALSE}
devtools::load_all()
knitr::read_chunk(here::here("code", "topic-distribution.R"))
```
...
```{r  plot-30-topics, eval = TRUE, echo = FALSE, fig.height = 7, fig.width = 9}
```